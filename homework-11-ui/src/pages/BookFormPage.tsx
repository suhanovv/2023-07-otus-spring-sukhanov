...
export default function BookFormPage() {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                currentBook = bookId ? await bookRepo.get(bookId) : undefined
            } catch (e) {
                return navigate("/404")
            }
    ...
    return (
        <>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <SelectControl fieldName="authorId" fieldLabel="Автор" defaultValue={formState.authorId} data={authors} optionText={(i: Author) => i.lastName + ' ' + i.firstName } onChange={(e: any) => setFormState({...formState, 'authorId': e.target.value})} />
                <SelectControl fieldName="genreId" fieldLabel="Жанр" defaultValue={formState.genreId} data={genres} optionText={(i: Genre) => i.name } onChange={(e: any) => setFormState({...formState, 'genreId': e.target.value})} />
